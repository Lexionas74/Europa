...
class Europa(commands.Bot):
    def __init__(self, command_prefix=..., **options):
        super().__init__(command_prefix, **options)
        self.owner_ids = {687882857171255309, 690427663315238964}
        self.help_command = commands.MinimalHelpCommand()
        self.prefix_cache = {}
        self.launch_time = datetime.utcnow()
        self.description = "Europa"

    async def Prefix(self):
        await self.wait_until_ready()
        self.db = await aiosqlite.connect("prefix.db")
    ...
    async def Economy(self):
        await self.wait_until_ready()
        self.economy_db = await aiosqlite.connect("economy.db")
        await self.economy_db.execute(
            "CREATE TABLE IF NOT EXISTS economy(author_id int, wallet int, bank int, PRIMARY KEY (author_id))"
        )
        await self.economy_db.commit()
